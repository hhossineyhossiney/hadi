...
function RankingReview({data,setData,updateScore,save,saving,onBack,onReminder}:any){
  ...
  return <div className="space-y-5"><div className="flex items-center justify-between"><div><button onClick={onBack} className="mb-2 text-xs text-cyan-300">← بازگشت</button><h2 className="text-2xl font-black">پرونده {data.academyName||m.name}</h2><p className="text-xs text-slate-400">سال {data.year} • امتیاز لحظه‌ای {total.toFixed(2)} از ۱۰۰</p></div><div className="flex h-20 w-20 items-center justify-center rounded-full border-8 border-cyan-400/20 text-xl font-black">{Math.round(total)}</div></div>
    ...
    <section className="rounded-[18px] border border-cyan-400/15 bg-gradient-to-l from-cyan-500/[0.07] to-transparent p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><div className="text-[10px] font-black text-cyan-200">مشخصات آموزشگاه</div><h3 className="mt-1 text-xl font-black text-white">{m.name}</h3><p className="mt-2 max-w-3xl text-[11px] leading-7 text-slate-300">{m.description}</p></div><div className="rounded-xl bg-black/20 px-4 py-3 text-center"><div className="text-[9px] text-slate-500">کد آموزشگاه</div><div className="mt-1 font-black text-cyan-200" dir="ltr">{m.academyCode}</div></div></div><div className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-4"><InfoValue icon="📅" label="سال تأسیس" value={String(m.establishedYear)} compact/><InfoValue icon="👤" label="مدیر آموزشگاه" value={m.managerName} compact/><InfoValue icon="📍" label="آدرس" value={m.address} compact/><InfoValue icon="📞" label="شماره تماس" value={m.mobile!=="ثبت نشده"?m.mobile:m.phone} compact/></div></section>
