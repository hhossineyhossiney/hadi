...
function statusText(status: string) {
  return ({ submitted: "منتظر بررسی", under_review: "در حال بررسی", needs_correction: "نیازمند اصلاح", approved: "تاییدشده", published: "منتشرشده" } as Record<string, string>)[status] || status;
}
...
export default function ExpertRankingDashboard() {
  ...
  const alerts = (list?.items || []).filter((item: any) => ["urgent", "high", "documents"].includes(item.priority)).slice(0, 6);
          ...
          <div className="space-y-5">
            ...
            <section className="relative overflow-hidden rounded-[24px] border border-cyan-400/15 bg-gradient-to-l from-[#0b3c62] via-[#092943] to-[#071b31] p-5 shadow-[0_25px_70px_rgba(0,0,0,0.25)]">
              <div className="relative grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  ...
                  <HeaderMetric icon="⚠" label="مدارک ناقص" value={stats.incompleteDocuments} warn />
            ...
            {alerts.length > 0 && (
              <DashboardCard title="هشدارها و موارد نیازمند اقدام" subtitle="مواردی که باید سریع‌تر بررسی شوند" icon={<AlertTriangle className="h-5 w-5 text-amber-300" />}>
                <div className="grid gap-2 md:grid-cols-2">{alerts.map((item: any) => <button key={item.id} onClick={() => open(item)} className="flex items-center gap-3 rounded-[14px] border border-amber-400/15 bg-amber-400/[0.055] p-3 text-right"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-400/10 text-xl">{item.priority === "documents" ? "📎" : item.priority === "urgent" ? "⏰" : "⚠️"}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-black">{item.academy_name}</span><span className="mt-1 block text-[10px] text-slate-400">{item.priority === "documents" ? "مستندات پرونده تکمیل نشده است" : item.priority === "urgent" ? "بیش از دو روز در انتظار بررسی" : "پرونده نیازمند اصلاح است"}</span></span><Eye className="h-4 w-4 text-amber-300" /></button>)}</div>
              </DashboardCard>
            )}
            ...
            <section className="overflow-hidden rounded-[22px] border border-white/10 bg-[#0b223b]">
              <div className="border-b border-white/10 p-4">
                ...
                <div className="mt-4 flex flex-col gap-2 sm:flex-row"><label className="flex flex-1 items-center gap-2 rounded-xl border border-white/10 bg-black/15 px-3"><Search className="h-4 w-4 text-slate-500" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="جست‌وجوی آموزشگاه یا شهر..." className="w-full bg-transparent py-3 text-xs outline-none" /></label>{showFilters && <><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rounded-xl bg-white/90 px-3 py-2.5 text-xs font-bold text-slate-900"><option value="all">همه وضعیت‌ها</option><option value="submitted">منتظر بررسی</option><option value="under_review">در حال بررسی</option><option value="needs_correction">نیازمند اصلاح</option><option value="published">منتشرشده</option></select><select value={rankFilter} onChange={(e) => setRankFilter(e.target.value)} className="rounded-xl bg-white/90 px-3 py-2.5 text-xs font-bold text-slate-900"><option value="all">همه رتبه‌ها</option>{["A+", "A", "B", "C", "D"].map((rank) => <option key={rank} value={rank}>{rank}</option>)}</select></>}</div>
              </div>
              <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-xs"><thead><tr className="border-b border-white/10 text-right text-[10px] font-black text-slate-500"><th className="p-4">مقایسه</th><th>آموزشگاه</th><th>رتبه</th><th>امتیاز</th><th>قبولی</th><th>هنرجو</th><th>وضعیت</th><th>اولویت</th><th>عملیات</th></tr></thead><tbody>{items.map((item: any) => { const score = Number(item.score || 0); const theme = scoreTheme(score); return <tr key={`${item.id}-${item.academy_id}`} className="border-b border-white/5 transition hover:bg-white/[0.03]"><td className="p-4"><input type="checkbox" checked={compareIds.includes(Number(item.academy_id))} onChange={() => setCompareIds((current) => current.includes(Number(item.academy_id)) ? current.filter((id) => id !== Number(item.academy_id)) : current.length < 2 ? [...current, Number(item.academy_id)] : [current[1], Number(item.academy_id)])} /></td><td><div className="font-black text-white">{item.academy_name}</div><div className="text-[9px] text-slate-500">{item.city || "—"}</div></td><td><span className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 font-black ${theme.bg} ${theme.text}`}><span className={`h-2.5 w-2.5 rounded-full ${theme.dot}`} />{item.rank || theme.label}</span></td><td className={`text-base font-black ${theme.text}`}>{score}</td><td>{Number(item.pass_rate || 0).toLocaleString("fa-IR")}٪</td><td>{Number(item.total_students || 0).toLocaleString("fa-IR")}</td><td><span className="rounded-full bg-white/5 px-2 py-1 text-[10px]">{statusText(item.status)}</span></td><td><PriorityBadge value={item.priority} /></td><td><button onClick={() => open(item)} className="rounded-lg bg-primary-600 px-3 py-2 font-black">باز کردن پرونده</button></td></tr>; })}{items.length === 0 && <tr><td colSpan={9} className="p-12 text-center text-slate-500">پرونده‌ای با این فیلتر پیدا نشد.</td></tr>}</tbody></table></div>
            </section>
...
function PriorityBadge({ value }: { value: string }) {
  const config = value === "urgent" ? ["فوری", "bg-rose-500/15 text-rose-300"] : value === "high" ? ["بالا", "bg-amber-500/15 text-amber-300"] : value === "documents" ? ["مدارک ناقص", "bg-fuchsia-500/15 text-fuchsia-300"] : ["عادی", "bg-slate-500/10 text-slate-400"];
