...
export default function OnlineCourseCard({
  ...
  const discount = Number(course.discount_percent) || (hasDiscount ? Math.round(((original - price) / original) * 100) : 0);
    ...
    >
      <Link href={`/shop/${course.slug}`} className="relative block h-full group">
        ...
        <article
          ...
          style={{ borderRadius: "48px 28px 48px 28px / 32px 48px 32px 48px" }}
        >
          <div
          ...
          <div className="relative flex items-start gap-3 p-5 pb-3">
            ...
            <div className="flex-1 min-w-0 flex flex-col">
              ...
              {course.subtitle && (
                <p className="text-[11px] text-slate-400 leading-relaxed line-clamp-2">{course.subtitle}</p>
              )}
            </div>
          </div>
          ...
          <div className="relative px-5 py-2 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                {course.instructor && <div className="text-[11px] font-black text-white truncate">مدرس: {course.instructor}</div>}
                <div className="text-[9.5px] text-slate-500 truncate">{course.institute_name || "آموزشگاه معتبر"}</div>
              </div>
            </div>
            {course.level && (
          ...
          <div className="relative mt-auto p-5 pt-4 flex items-end justify-between gap-3">
            <div className="min-w-0">
              ...
              {hasDiscount && <div className="text-[10px] text-slate-500 line-through leading-none" dir="ltr">{fmt(original)}</div>}
