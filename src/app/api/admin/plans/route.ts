...
import { ensureSubscriptionEntitlementsSchema, syncLegacySellablePermission } from "@/lib/subscription-entitlements";
...
export async function POST(req: Request) {
  ...
  try {
    if (action === "createPlan") {
      const { name, slug, description, price, priceYearly, durationDays, maxCourses, maxStudents, maxShopCourses, onlineSalesEnabled, commissionPercent, features, color, isPopular, sortOrder } = body;
      ...
      const [p] = await db.execute(sql`
        ...
        VALUES (${name}, ${slug}, ${description || null}, ${String(price)}, ${priceYearly ? String(priceYearly) : null},
                ...
                ${!!onlineSalesEnabled}, ${String(commissionPercent || "10.00")}, ${JSON.stringify(features || [])}::jsonb,
    ...
    if (action === "updatePlan") {
      const { id, name, description, price, priceYearly, durationDays, maxCourses, maxStudents, maxShopCourses, onlineSalesEnabled, commissionPercent, features, color, isPopular, isActive, sortOrder } = body;
      await db.execute(sql`
        ...
        WHERE id = ${Number(id)}
      `);
      const subscriberRows = await db.execute(sql`SELECT institute_id FROM institute_subscriptions WHERE plan_id = ${Number(id)} AND status IN ('active', 'trial')`);
      const subscribers = ((subscriberRows as any).rows || subscriberRows) as { institute_id: number }[];
      await Promise.all(subscribers.map((item) => syncLegacySellablePermission(Number(item.institute_id), adminId)));
    ...
    if (action === "assignPlan") {
      ...
      const pRow = await db.execute(sql`SELECT duration_days, price FROM subscription_plans WHERE id = ${Number(planId)} AND is_active = true`);
      ...
      const [sub] = await db.execute(sql`
        ...
        RETURNING *
      `).then((r: any) => (r.rows || r));
      // Online sales permission, course limit, and commission are inherited
      // automatically from the assigned plan. No second admin action is needed.
      const entitlement = await syncLegacySellablePermission(Number(instituteId), adminId);
      return NextResponse.json({ ok: true, subscription: sub, entitlement });
    }
